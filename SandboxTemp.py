...
def startMsg2(dis):
    value = score_font.render("rotate 360 deg or SPACE bar multiple times to rotate manually", True, black)
    dis.blit(value, [25, dis_height / 2])
...
def startMsg4(dis):
    value = score_font.render("use 1/2/3/4/5/6/r keys to change color", True, black)
    dis.blit(value, [110, 50])
...
def rotate3D_Z(v,x,c):
    arr2 = [math.cos(x)*(v[0]-c[0]) + math.sin(x)*(v[1]-c[1]),-math.sin(x)*(v[0]-c[0]) + math.cos(x)*(v[1]-c[1]), v[2]]
    arr2[0] = arr2[0] + c[0]
    arr2[1] = arr2[1] + c[1]
    return arr2

def rotate3D_Y(v,x,c):
    arr2 = [math.cos(x)*(v[0]-c[0]) + math.sin(x)*(v[2]-c[2]),v[1],-math.sin(x)*(v[0]-c[0]) + math.cos(x)*(v[2]-c[2])]
    arr2[0] = arr2[0] + c[0]
    arr2[2] = arr2[2] + c[2]
    return arr2

def rotate3D_X(v,x,c):
    arr2 = [v[0],math.cos(x)*(v[1]-c[1]) + math.sin(x)*(v[2]-c[2]),-math.sin(x)*(v[1]-c[1]) + math.cos(x)*(v[2]-c[2])]
    arr2[1] = arr2[1] + c[1]
    arr2[2] = arr2[2] + c[2]
    return arr2
